<template>
    <div v-if="contractModel.workflowType === 'create_sale_contract'">
        <h4 class="ml50">合同附加条款</h4>
        <template v-if="hasInsurance()">
            <el-form-item label="商业保险" prop="vciBuyType"
                          :rules="{required: true, message: '请选择商业保险', trigger: 'change'}">
                <el-radio-group  v-model="contractModel.contractForm.vciBuyType">
            ...
            <el-form-item label="保险计划名称" prop="vciPlanName"
                          :rules="{required: true, message: '请输入保险计划名称', trigger: 'blur'}">
                <el-input v-model="contractModel.contractForm.vciPlanName"></el-input>
            </el-form-item>
        ...
        <el-form-item label="付款方式" prop="vciPayType"
                      :rules="{required: true, message: '请输入付款方式', trigger: 'change'}">
            <el-select v-model="contractModel.contractForm.vciPayType" placeholder="请选择" style="width:100%;">
                <el-option v-for="(item,key) in contractModel.VciPayTypeList" :key="key" :label="item.text" :value="item.value"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item prop="vciSettleExp" v-if="contractModel.contractForm.vciPayType == 30"
        ...
        <el-form-item label="付款方与收票方一致" prop="payAndInvoiceSame"
                      :rules="{required: true, message: '请选择付款方与收票方一致', trigger: 'change'}">
            <el-radio-group v-model="contractModel.contractForm.payAndInvoiceSame">
                <el-radio label="1">是</el-radio>
            </el-radio-group>
            <el-radio-group class="ml20" v-model="contractModel.contractForm.payAndInvoiceSame">
        ...
        <el-form-item v-show="contractModel.contractForm.payAndInvoiceSame === '0'" label="甲方下属公司类型"
                      :rules="{required: true, message: '请选择甲方下属公司类型', trigger: 'blur'}">
            <el-radio v-model="contractModel.contractForm.customCompanyUnderType"
                      v-for="(e, key) in customCompanyUnderTypeList" :key="key" :label="e.value">{{e.text}}</el-radio>
        </el-form-item>
