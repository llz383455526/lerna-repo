  ...
  <div>
    ...
    >
      ...
      <el-button
        size="small"
        type="info"
        @click="toCreate(contractModel.contractId, 'edit')"
      >编辑</el-button>
      <el-button
        size="small"
        type="danger"
      ...
      <div class="widget-body">
        ...
        >
          ...
          >
            ...
            <div class="col-xs-6">合同模板：{{ getText(contractForm.contractTplId, contractModel.contractTplList) }}</div>
            <div class="col-xs-6">客户行业类型：{{ getText(contractForm.contractType, contractModel.businessTypeList) }}</div>
            <div class="col-xs-6">客户从事：{{ contractForm.customIndustry }}</div>
            <div class="col-xs-6">合同期限：{{ contractForm.contractStartDate + ' - ' + contractForm.contractEndDate }}</div>
            <div class="col-xs-12">服务类型：{{ getCheckText(contractForm.serviceType, contractModel.serviceTypes, 'serviceName') }}</div>
          </div>
          <div
            class="row"
            style="margin-bottom: 15px;"
          ...
          >
            ...
            <div class="col-xs-12">客户性质：{{ getText(contractForm.customNature, contractModel.customNatureList) }}</div>
          ...
          >
            ...
            >
              ...
              <div class="col-xs-12">结算方式：{{ getText(formItem.settleType, contractModel.settleTypeList) }}</div>
              <div class="col-xs-12">业务方案：{{ getText(formItem.goodsId, formItem.goodsList, 'id', 'name') }}</div>
              <div class="col-xs-12">发放方式：{{ getPayMode(formItem.channelTypeList, contractModel.payMode) }}</div>
              <div class="col-xs-12">
          ...
          >
            ...
            <template v-if="hasInsurance()">
              <div class="col-xs-12">商业保险：{{ contractForm.vciBuyType == 10 ? '由客户自行购买' : '我方购买保险的情况，无论保费是否我方承担均适用'}}</div>
              <div class="col-xs-12">保险计划名称：{{ contractForm.vciPlanName }}</div>
            </template>
            <div class="col-xs-12">付款方式：{{ getText(contractForm.vciPayType, contractModel.VciPayTypeList) }}</div>
            <div class="col-xs-12">付款方与收款方一致：{{ contractForm.payAndInvoiceSame == 1 ? '是' : '否' }}</div>
            <div
              class="col-xs-12"
              v-if="contractForm.payAndInvoiceSame === '0'"
            >甲方下属公司类型：{{ getText(contractForm.customCompanyUnderType, contractModel.customCompanyUnderTypeList) }}</div>
            <div
              class="col-xs-12"
              v-if="contractForm.payAndInvoiceSame === '0'"
            >甲方下属公司清单：
