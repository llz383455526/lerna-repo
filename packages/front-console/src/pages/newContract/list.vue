<template>
  <div class="main-container">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="我的入驻申请" name="first" />
      <el-tab-pane label="待处理入驻申请" name="second" />
      <el-tab-pane label="全部入驻申请" name="third" />
    </el-tabs>
    ...
    <el-button size="small" @click="$router.push({path:'create',query:{workflowType:'create_sale_contract'}})">
      创建合同
    </el-button>
    <el-button size="small" @click="$router.push({path:'create_add'})">
      补签合同
    </el-button>
    ...
    <el-button size="small" @click="$router.push({path:'create_change',query:{workflowType:'update_sale_contract'}})">
      合同变更
    </el-button>
    ...
    <div class="table-container">
      <el-table :data="tableList.data">
        ...
        <el-table-column prop="customerName" label="企业名称" />
        <el-table-column prop="createdByName" label="申请人" />
        <el-table-column prop="updatedAt" label="提交时间" />
        <el-table-column prop="curProcessUser" label="当前处理人" />
        <el-table-column prop="processedAt" label="最后审批时间" />
        <el-table-column prop="standardEnumString" label="合同类型">
        ...
        <el-table-column label="操作" width="200">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status != 'draft'" @click="toPreview(scope.row.id, 'watch')" type="text" size="medium" style="padding:0;">
              查看
            </el-button>
            <el-button v-if="scope.row.status === 'init' || scope.row.status === 'draft'" @click="toCreate(scope.row.id, scope.row.workflowType)" type="text" size="medium" style="padding:0;">
              编辑
            </el-button>
            <el-button v-if="scope.row.status === 'init' && scope.row.createdBy == userInformation.id" @click="toDetail(scope.row.id, 'watch')" type="text" size="medium" style="padding:0;">
              送审
            </el-button>
            <el-button v-if="scope.row.status === 'draft' || scope.row.status === 'init'" @click="closeContract(scope.row.id)" type="text" size="medium" style="padding:0;">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <ayg-pagination v-if="tableList.total" :total="tableList.total" @handleSizeChange="handleSizeChange" :current-size="pageSize" @handleCurrentChange="handleCurrentChange" :current-page="pageIndex" />
  </div>
</template>
