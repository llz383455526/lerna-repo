        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <span @click="upFileClick(scope.row)" style="color: rgb(26, 179, 148); cursor: pointer;"
                          v-else-if="!scope.row.riskRateDataServiceDetailDTOList || scope.row.riskRateDataServiceDetailDTOList.length === 0">未上传 <i
                        class="el-icon-upload2"></i></span>
                    <span @click="upFileClick(scope.row)" style="color: rgb(26, 179, 148); cursor: pointer;" v-else>可上传 <i
                        class="el-icon-upload2"></i></span>
            ...
            <el-table-column
                label="落地公司">
                <template slot-scope="scope">
                    <ul class="can-upload-companys"
                        v-if="scope.row.uploadServiceCompanyList && scope.row.uploadServiceCompanyList.length > 0">
                        <li class="item" v-for="(v, k) in scope.row.uploadServiceCompanyList" :key="k">{{v.name}}</li>
                    </ul>
                    <ul class="already-upload-companys"
                        v-if="scope.row .riskRateDataServiceDetailDTOList && scope.row.riskRateDataServiceDetailDTOList.length > 0">
                        <li class="item" v-for="(v, k) in scope.row.riskRateDataServiceDetailDTOList" :key="k">
                            <el-row :gutter="20">
                                ...
                                <el-col :span="2">
                                    ...
                                    <span class="preview" v-if="v.canDelete" @click="removeBtnClick(v)">删除</span>
                                </el-col>
        ...
        methods: {
            ...
            removeBtnClick(item) {
                this.$confirm('确定要删除嘛?', '提示', {
                    ...
                    get('/api/console-dlv/risk-level-approve/delete-cuser-performance', {
                        id: item.id
                    }).then((data) => {
                        this.$emit('reload')
                    }).catch(() => {
