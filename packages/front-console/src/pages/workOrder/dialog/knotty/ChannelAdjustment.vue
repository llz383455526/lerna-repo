<template>
    <div>
        ...
        >
            ...
            <el-form-item
              label="商户名称"
              prop="transferMerchantId"
            >
                <el-select
                  class="form_input"
                  v-if="isCreate"
                  v-model="form.transferMerchantId"
                  filterable
                  @change="selectTransferMerchant"
                >
                  <el-option
                    v-for="item in productName"
                    :key="item.value"
            ...
            >
                ...
                >
                  <el-option
                    v-for="item in transferChannelName"
                    :key="item.value"
                    :label="item.text"
                    :value="item.value"
                  ></el-option>
...
export default {
    ...
    data() {
        return {
            form: {
                ...
                "executionId": "", // 任务执行ID
                "id": "", // 业务ID
                "balanceAccountId": "", // 转出商户余额账户ID
                "merchantId": "", // 商户ID
                "transferMerchantId": "", // 转入商户id
                "transferMerchantName": "", // 转入商户名称
                "merchantName": "", // 商户名称
                ...
                "transferBalanceAccountId": "", // 转入商户余额账户ID
                channelInfo: {
                  channelAlias: '',
                  channelLoginAcctNo: '',
                  channelMerCustId: '',
            ...
            transferChannelList: [], // 转入渠道账号
            transferChannelName: [], // 转入业务类型
    ...
    watch: {
        //选择客户公司
        /**
         *  1. 将this.form['merchantId','transferMerchantId',
             'serviceCompanyId', 'businessId', 'transferBusinessId', 'channelAccount', 'transferChannelAccount']置空
            2. this.form.customerCompanyName 赋值为 companyName
         */
      'form.customerCompanyId': function(newVal) {
        this.watchFun(newVal, '', ['merchantId','transferMerchantId',
             'serviceCompanyId', 'businessId', 'transferBusinessId', 'channelAccount', 'transferChannelAccount'],
      ...
      // 选择转入商户id
      'form.transferMerchantId': function(newVal) {
        this.watchFun(newVal, '', ['transferBusinessId', 'transferChannelAccount'],
           'productName', 'transferMerchantName')
      },
      // 选择服务公司
      'form.serviceCompanyId': function(newVal) {
        this.watchFun(newVal, '', ['merchantId', 'transferMerchantId',
         'businessId', 'transferBusinessId', 'channelAccount', 'transferChannelAccount'],
      ...
      'form.transferBusinessId': function(newVal) {
        this.watchFun(newVal, '', ['transferChannelAccount'],
           'transferChannelName', 'transferBusinessName')
      },
      // 选择转出渠道
      'form.channelAccount': function(newVal) {
        ...
        this.watchFun(newVal, '', [],
           'channlList', 'balanceAccountId', 'balanceAccountId', 'balanceAccountId')
      },
      // 选择转入渠道
      'form.transferChannelAccount': function(newVal) {
        ...
        this.watchFun(newVal, '', [],
           'transferChannelList', 'transferBalanceAccountId', 'balanceAccountId', 'balanceAccountId')
      },
    ...
    methods: {
         ...
         * @param idKey id字段，用来判断
         * @param nameKey name字段 用来赋值
         * 
         */
        ...
        selectCompany(val) {
            this.getServiceList({companyId: val})
        ...
        selectTransferMerchant(val) {
            this.getBusinessList({
                appId: val,
                serviceCompanyId: this.form.serviceCompanyId,
                cb: true
            }).then(res => {
                this.transferChannelName = res
            })
        },
        ...
        selectTransferBusiness(val) {
            this.getChannlList({
                appId: this.form.transferMerchantId,
                serviceCompanyId: this.form.serviceCompanyId,
                bankType: val,
                cb: true
            }).then(res => {
                this.transferChannelList = res
            })
        },
        result(res) {
            let arr = []
