<template>
    <div>
        <el-dialog title="" :visible.sync="show" width="800px">
            <div class="tip">
            ...
            <el-steps :active="active" align-center class="mt20 mb20" :key="stepList.length">
                <el-step v-for="e in stepList" :key="e.activityId" :title="e.activityName" :description="e.userName"></el-step>
            </el-steps>
            <div class="title mb20 green_0">状态：{{msg.statusName}}</div>
            <div v-if="currentStpe.activityId == 'taskApply' || currentStpe.activityId == 'taskAudit'">剩余票量：<span class="red">{{amount}}</span>张</div>
            <el-table class="mt20" :data="[msg.orderTitle]">
                <el-table-column label="客户名称" prop="customCompanyName"></el-table-column>
                <el-table-column label="开票公司" prop="serviceCompanyName"></el-table-column>
                <el-table-column label="开票类型" prop="invoiceTypeName"></el-table-column>
                <el-table-column label="开票总金额">
                    <template slot-scope="scope">
                        {{scope.row ? scope.row.amount : 0 | formatMoney}}
                    </template>
                </el-table-column>
                <el-table-column label="申请发票数" v-if="currentStpe.activityId == 'taskApply' || currentStpe.activityId == 'taskAudit'" :prop="currentStpe.activityId == 'taskApply' ? 'totalNum' : 'realNum'"></el-table-column>
                <el-table-column label="寄送发票数" v-else prop="realNum"></el-table-column>
            </el-table>
            <template v-if="currentStpe.activityId == 'taskApply' || currentStpe.activityId == 'taskAudit' || currentStpe.activityId == 'taskCreate' || currentStpe.activityId == 'taskExceptionCheck'">
                <el-table class="mt20" :data="msg.orderItems" @selection-change="selectionChange" key="1">
                    <el-table-column type="selection" v-if="currentStpe.activityId == 'taskAudit' && !look" :selectable="selecCheck"></el-table-column>
                    <el-table-column label="序号" prop="orderItemId"></el-table-column>
                    <el-table-column label="开票类目">
                        <template slot-scope="scope">
                            <div v-for="(e, i) in scope.row.invoiceSubjects" :key="e">{{e}}</div>
                        </template>
                    </el-table-column>
                ...
                <el-form :model="form" label-width="120px" ref="form">
                    <el-form-item label="发票用途" v-if="msg.purpose"> <!-- !look || stepList.length == 2 -->
                        {{msg.purpose}}
                    </el-form-item>
                    <el-form-item label="发票显示备注" v-if="msg.remark">
                        {{msg.remark}}
                    </el-form-item>
                    <el-form-item label="备注" v-if="(currentStpe.activityId == 'taskApply' && !look) || (msg.busiRemark && look)" prop="busiRemark">
                        <el-input class="form_input" v-model="form.busiRemark" type="textarea" v-if="currentStpe.activityId == 'taskApply' && !look"></el-input>
                        <template v-else-if="msg.busiRemark && look">{{msg.busiRemark}}</template>
                    </el-form-item>
                </el-form>
            </template>
            <template v-else-if="stepList.length == 5">
                <div class="title">上传发票扫描件</div>
                <!-- <el-upload
                ...
                <el-table :data="msg.orderItems" key="2">
                    <el-table-column label="序号" prop="orderItemId"></el-table-column>
                    <el-table-column label="开票类目">
                        <template slot-scope="scope">
                            <div v-for="(e, i) in scope.row.invoiceSubjects" :key="e">{{e}}</div>
                    ...
                    <el-table-column label="发票代码" prop="invoiceCode">
                        <template slot-scope="scope">
                            ...
                            <el-input v-else v-model="scope.row.invoiceCode"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column label="发票号码" prop="invoiceNo">
                        <template slot-scope="scope">
                            ...
                            <el-input v-else v-model="scope.row.invoiceNo"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column label="开票状态" prop="stateName">
                        <template slot-scope="scope">
                            <span :class="scope.row.state == 'fail' ? 'red' : 'green_0'">{{scope.row.stateName}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="发票扫描件" prop="invoiceVoucherName">
                        <template slot-scope="scope">
                            <span v-if="!look">{{scope.row.invoiceVoucherName}}</span>
                            <a v-else-if="scope.row.invoiceVoucher && scope.row.state != 'fail'" :href="`${invoiceApi.preview}?ossId=${scope.row.invoiceVoucher}`" target="_bank">
                    ...
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            <el-upload
                                ...
                                :show-file-list="false"
                                accept="image/*, .pdf">
                                <el-button type="text" @click="targetId = scope.row.trasactionOrderItemId">上传文件</el-button>
                            </el-upload>
                            <a v-else-if="scope.row.invoiceVoucher" :href="`${invoiceApi.download}?ossId=${scope.row.invoiceVoucher}`" :download="scope.row.invoiceVoucherName" target="_bank">
                ...
                <div class="title">寄送发票</div>
                <el-button class="mt20" size="small" type="primary" @click="showInput" v-if="!look">录入物流信息</el-button>
                <el-table :data="[inputForm]">
                    ...
                    <el-table-column label="物流单号" prop="expressOrderNo"></el-table-column>
                    <el-table-column label="寄送凭证" prop="expressVoucherName">
                        <template slot-scope="scope">
                            <span v-if="!look">{{scope.row.expressVoucherName}}</span>
                            <a v-else-if="scope.row.expressVoucher" :href="`${invoiceApi.preview}?ossId=${scope.row.expressVoucher}`" target="_bank">
                    ...
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            <el-button type="text" v-if="!look && inputForm.expressCompany" @click="showInput(true)">编辑</el-button>
                            <a v-else-if="scope.row.expressVoucher" :href="`${invoiceApi.download}?ossId=${scope.row.expressVoucher}`" :download="scope.row.expressVoucherName" target="_bank">
                                <el-button type="text">下载</el-button>
                            </a>
                        </template>
                    </el-table-column>
            ...
            <div v-for="(e, i) in realStep" :key="e.activityId">
                ...
                <template v-if="stepList.length == 5 || look">
                    ...
                    <template v-if="!look">
                        ...
                        <template v-if="currentStpe.activityId == 'taskAudit'">
                            <el-button type="primary" @click="makeInvoice">开票</el-button>
                        </template>
                    ...
                    <template v-else-if="(currentStpe.activityId == 'taskApply' || currentStpe.activityId == 'taskAudit') && isMe">
                        <el-button type="primary" v-if="param.taskName != '结束'" @click="cancel">取消申请</el-button>
                    </template>
                </template>
                <template v-else>
                    ...
                    <el-button v-else @click="show = false">关闭</el-button>
                </template>
            </span>
        </el-dialog>
        <audit-reject @reject="reject" ref="auditReject"></audit-reject>
        <input-msg @result="inputResult" ref="inputMsg"></input-msg>
    </div>
</template>
<script>
