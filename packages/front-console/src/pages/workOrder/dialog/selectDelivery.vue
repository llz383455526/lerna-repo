<template>
    <div>
        <el-dialog title="关联交付" :visible.sync="show" width="800px">
...
export default {
    ...
    methods: {
        ...
        query(a) {
            ...
            this.form.page = a
            post(sysmgr.relationGroupEmployeeList, this.form).then(data => {
                this.listData = data
                this.$nextTick(() => {
        ...
        selectable(a) {
            return !this.deliveryList.filter(e => e.id == a.id).length
        },
        sure() {
            this.show = false
            this.$emit('result', this.deliveryList.concat(this.selectList))
        }
    }
}
</script>
